...
### logistica
var_total_logit_adapt_excl<- nls(var_total_adapt_excl~(a/(1+exp(-b*(dist_adapt_excl-c)))),start=list(a=0.0016,b=0.0002,c=5000))
funcao_logit_mle_normal <- function(a,b,c,sd){
  mean <- a/(1+exp(-(b*(dist_adapt_excl-c))))
  -sum(dnorm(var_total_adapt_excl, mean=mean, sd=sd, log=TRUE))
}
start=list(a=0.0016,b=0.0002,c=5000,sd=sd(var_total_adapt_excl))
...
start=list(a=0.921,b=0.000035,c=90000,shape=mean(var_total_adapt_excl)^2/var(var_total_adapt_excl))
...
var_total_quadr_adapt_excl_mle_gama_nm <- mle2(funcao_quadr_mle_gama, start=start,method = "Nelder-Mead",control=list(maxit=1000))
...
var_total_holling_adapt_excl_mle_gama_nm <- mle2(funcao_holling_mle_gama, start=start,method = "Nelder-Mead",control=list(maxit=1000))
...
AICtab(var_total_nulo_adapt_excl_mle_normal_nm,var_total_nulo_adapt_excl_mle_gama_nm)
...
curve((coef(var_total_logit_adapt_excl)[[1]]/(1+exp(-(coef(var_total_logit_adapt_excl)[[2]]*(x-coef(var_total_logit_adapt_excl)[[3]]))))),add=T,col="black")
