...
simula.parallel <- function(replica) {
  res <- simula.neutra.trade(S = dados[replica,1],
                             ...
                             X = 20000,
                             dp = 500,
                             dist.pos = if(dados[replica,2]>0 & dados[replica,2]<3e5) round(seq(from = 3e5/(dados[replica,2]+1), to = 3e5-(3e5/(dados[replica,2]+1)), length.out = dados[replica,2])) else if(dados[replica,2]==0) NULL else seq(1,3e5,1),
...
for (i in (seq(1,1000,8)[111:125]))
{
  replica.sim <- as.list(i:(i+7))
  resultados <- llply(.data = replica.sim, .fun = simula.parallel, .parallel = TRUE)
  save(resultados,file=paste("resultados4mar17_dp500_+ind_",i,"-",i+7,".RData",sep=""))
}