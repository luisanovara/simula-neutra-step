...
simula.neutra.trade=function(S= 100, j=10, xi0=rep(seq(10,10,length.out = S),each=j), X=10000, dp=0, dist.pos=NULL, dist.int=NULL, ciclo=1e6, step=100)
{
  ...
  for(i in 1:ciclo)
  {
    ...
    if(n.mortes>0) ### Identifica se houve mortes no ciclo
    {
      ...
      ### FIM DA PARTE TESTE
      medias.prop <- n.propag[mami] ### Calcula o valor de propagulos produzidos por ciclo dos filhotes, representado pela media do numero medio de propagulos produzidos pelos parentais
