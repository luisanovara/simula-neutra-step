...
pos_comite_dp0_riqueza_temporal <- matrix(ncol=3001,nrow=1000)
pos_comite_dp0_mortes_cumulativas_temporal <- matrix(ncol=3001,nrow=1000)
...
for(i in seq(from=1,to=1000,by=8)){ 
  ...
  for(j in 1:length(x)){
    ...
    desv_entre[1,which(is.nan(desv_entre[1,])==T)]<-0
    # riqueza
  ...
  pos_comite_dp0_riqueza_temporal[i:(i+7),] <- riqueza_8
  pos_comite_dp0_mortes_cumulativas_temporal[i:(i+7),] <- mortes_8
}
...
save(pos_comite_dp0_riqueza_temporal,file="pos_comite_15fev17_dp0_riqueza_temporal.RData")
save(pos_comite_dp0_mortes_cumulativas_temporal,file="pos_comite_15fev17_dp0_mortes_cumulativas_temporal.RData")
