...
simulacoes_25mar16_output_geracao<-c()
for(i in 1:34){
  load(paste("sim_25mar16_",i,".RData",sep=""))
  simulacoes_25mar16_output_geracao[(3*i-2):(3*i)]<-simula_output(get(ls()[3]))
  rm(list=ls()[3])
}
simulacoes_25mar16_output_geracao<-simulacoes_25mar16_output_geracao[c(-101,-102)]
simulacoes_output<-simulacoes_25mar16_output_geracao
...
resulta_geral$Dif_Media_xi0<-resulta_geral[,1]-dados3_27abr16[,2]
### Output #9: diferenca entre a estrategia de vida final e a inicial / inicial
resulta_geral$Razao_Media_xi0<-(resulta_geral[,1]-dados3_27abr16[,2])/dados3_27abr16[,2]
