...
### logistica
var_total_logit_excl <- nls(var_total_excl~(a/(1+exp(-(b*(dist_excl-c))))),start=list(a=0.1,b=-0.003,c=2000))
funcao_logit_mle_normal <- function(a,b,c,sd){
  mean <- (a/(1+exp(-(b*(dist_excl-c)))))
  -sum(dnorm(var_total_excl, mean=mean, sd=sd, log=TRUE))
}
start <- list(a=0.1,b=-0.003,c=2000,sd=sd(var_total_excl))
...
start=list(a=0.921,b=0.000035,c=90000,shape=mean(var_total_excl)^2/var(var_total_excl))
...
AICtab(var_total_nulo_excl_mle_normal_nm,var_total_nulo_excl_mle_gama_nm)
...
axis(2,las=1,at=c(-1,0,0.02,0.04,0.06,0.08,0.1,1),labels=c("","0",expression("2"%.%10^"-2"),expression("4"%.%10^"-2"),expression("6"%.%10^"-2"),expression("8"%.%10^"-2"),expression("10"%.%10^"-2"),""),cex.axis=0.9)
