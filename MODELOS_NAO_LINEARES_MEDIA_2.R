...
media_mm_excl_mle_normal_nm <- mle2(funcao_mm_mle_normal, start=list(a=1/media_coef_mm_linearizada_excl[[2]],b=media_coef_mm_linearizada_excl[[1]]*(1/media_coef_mm_linearizada_excl[[2]]),c=0.6,sd=sd(media_excl)),method = "Nelder-Mead")
media_mm_excl_mle_normal_nm <- mle2(funcao_mm_mle_normal, start=list(a=0.45,b=350,c=0.55,sd=sd(media_excl)),method = "Nelder-Mead")
...
media_logit_excl<- nls(media_excl~(a/(1+exp(-(b*(dist_excl-c))))),start=list(a=1,b=0.0005,c=60))
funcao_logit_mle_normal <- function(a,b,c,sd){
  mean <- a/(1+exp(-(b*(dist_excl-c))))
  -sum(dnorm(media_excl, mean=mean, sd=sd, log=TRUE))
}
start=list(a=1,b=0.0005,c=60,sd=sd(media_excl))
...
start=list(a=0.921,b=0.000035,c=90000,shape=mean(media_excl)^2/var(media_excl))
...
AICtab(media_nulo_excl_mle_normal_nm,media_nulo_excl_mle_gama_nm)
