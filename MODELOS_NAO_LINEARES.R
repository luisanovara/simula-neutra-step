...
curve((coef(media_logit_adapt_2)[[1]]/(1+exp(-(coef(media_logit_adapt_2)[[2]]*(x-coef(media_logit_adapt_2)[[3]])))))+coef(media_logit_adapt_2)[[4]],add=T,col="green")
curve((1/(1+exp(-(coef(media_logit_adapt)[[1]]*x)-coef(media_logit_adapt)[[2]])))+coef(media_logit_adapt)[[3]],add=T,col="green4")
...
media_logit_excl_forca_bruta<- nls2(media_excl~((a/(1+exp(-(b*(dist_excl-c)))))+d),start=list(a=0.67,b=0.7,c=1,d=0.33),algorithm = "brute-force")
media_logit_excl_2<- nls(media_excl~((a/(1+exp(-(b*(dist_excl-c)))))+d),start=list(a=coef(media_logit_excl_forca_bruta)[[1]],b=coef(media_logit_excl_forca_bruta)[[2]],c=coef(media_logit_excl_forca_bruta)[[3]],d=coef(media_logit_excl_forca_bruta)[[4]]))
...
media_logit_excl_4<- nls2(media_excl~((a/(1+exp(-(b*(dist_excl-c)))))),start=list(a=data.frame(a=c(0.67,0.6,0.7,0.8,1,1)),b=data.frame(b=c(0.7,0.75,0.8,0.85,0.001822,1)),c=data.frame(c=c(rep(1,4),-0.467172/0.001822,1))),algorithm = "brute-force",trace=T)
## vamo de mle
funcao_logit_mle <- function(a,b,c,sd){
  mean <- (a/(1+exp(-(b*(dist_excl-c)))))
  -sum(dnorm(media_excl, mean=mean, sd=sd, log=TRUE))
}
start <- list(a=1,b=1,c=1,sd=sd(media_excl))
media_logit_excl_3 <- mle2(funcao_logit_mle, start=start,method="Nelder-Mead")
coef(media_logit_excl_3)
...
#### selecao
AICtab(media_nulo_excl,media_linear_excl,media_mm_excl,media_pot_excl,media_logit_excl,media_logit_excl_2,media_logit_excl_3,media_logit_excl_4,media_exp_excl)
#### plotando
...
curve((coef(media_logit_excl_3)[[1]]/(1+exp(-(coef(media_logit_excl_3)[[2]]*(x-coef(media_logit_excl_3)[[3]]))))),add=T,col="green4")
curve((coef(media_logit_excl_4)[[1]]/(1+exp(-(coef(media_logit_excl_4)[[2]]*(x-coef(media_logit_excl_4)[[3]]))))),add=T,col="green4")
...
#curve(2.25e-14*(x^1.95)+0.0002,add=T,col="blue")
var_total_pot_adapt <- nls(var_total_adapt~I(a*I(dist_adapt^b)+c),start=list(a=11e-4,b=0.01,c=0))
##
require(nlmrt)
...
#### selecao
AICtab(var_total_nulo_adapt,var_total_linear_adapt,var_total_mm_adapt,var_total_pot_adapt,var_total_logit_adapt,var_total_exp_adapt)
#### plotando
plot(var_total_adapt~dist_adapt,pch=20,col="gray",bty="l",xlim=c(0,3e5),ylim=c(0,0.0014))
curve((1/(1+exp(-(coef(var_total_logit_adapt)[[1]]*x)-coef(var_total_logit_adapt)[[2]])))+coef(var_total_logit_adapt)[[3]],add=T,col="green")
curve(((coef(var_total_mm_adapt)[[1]]*x)/(coef(var_total_mm_adapt)[[2]]+x))+coef(var_total_mm_adapt)[[3]],add=T,col="blue") #CORRIGIR?
...
curve(exp(coef(var_total_exp_adapt)[[1]]*x+coef(var_total_exp_adapt)[[2]]),add=T,col="violet") #CORRIGIR? 
